-- Q) Show the product_id(s), the sum of quantities, where the total sum of its product quantities is greater than or equal to 100.
SELECT product_id, SUM (quantity) AS total
FROM order_details
GROUP BY 1
HAVING SUM(quantity) >= 100;

-- Q) What was the city with the most profit for the company in 2015?
...
-- Q)In 2015, what was the most profitable city's profit?
SELECT o.shipping_city, sum(od.order_profits) AS total
FROM orders AS o
INNER JOIN order_details AS od
ON o.order_id = od.order_id
...
ORDER BY total DESC
LIMIT 5;
-- Q) How many different cities do we have in the data?
SELECT  count(DISTINCT shipping_city)
FROM orders;

-- Q) Show the total spent by customers from low to high.
...
-- Q) What is the most profitable city in the State of Tennessee?
...
-- Q) What’s the average annual profit for that city across all years?
...
-- Q) What is the distribution of customer types in the data?
SELECT customer_segment, COUNT(*) 
FROM customers
GROUP BY 1;

-- Q) What’s the most profitable product category on average in Iowa across all years?
...
-- Q) What is the most popular product in that category across all states in 2016?
...
-- Q) Which customer got the most discount in the data? (in total amount)
WITH t1 AS (
...
-- Q) How widely did monthly profits vary in 2018?
...
-- Q) Which order was the highest in 2015?
...
-- Q) What was the rank of each city in the East region in 2015?
...
-- Q) Display customer names for customers who are in the segment ‘Consumer’ or ‘Corporate.’ How many customers are there in total? 
...
-- Q) Calculate the difference between the largest and smallest order quantities for product id ‘100.’
SELECT MAX(quantity) - MIN(quantity) AS DIFF 
FROM order_details
WHERE product_id = 100

-- Q) Calculate the percent of products that are within the category ‘Furniture.’ 
SELECT COUNT(*) * 100 * 1.0 / (SELECT COUNT(*) FROM product)::numeric AS percentage
FROM product
WHERE product_category = 'Furniture'

-- Q) Display the number of duplicate products based on their product manufacturer.           
-- Example: A product with an identical product manufacturer can be considered a duplicate.
SELECT product_manufacturer, COUNT(*)
FROM product
GROUP BY 1;

-- Q) Show the product_subcategory and the total number of products in the subcategory.
...
--Q) How many customers do we have in the data?
